...
实机 GPADC0 阻值按键的确认结果为：K2 约 766 mV，对应 Linux
按键码 373（`KEY_MODE`）；K3 约 552–553 mV，对应按键码 119
（`KEY_PAUSE`）。配置中的 `*_alt` 保留旧版键值作为兼容映射。

## 操作方式

- K1（上）：移动到上一项、切换应用或向前翻页
- K2（下，按键码 373）：移动到下一项、切换应用或向后翻页
- K3（确定，按键码 119）短按：打开应用、执行当前操作
- K3 长按：逐级返回；阅读正文时先返回书库；在主页长按可切换中英文
...
确认时，短按 K3 接受、长按 K3 拒绝；屏幕会显示六位 passkey。
...
GB18030 转换。单文件上限 32 MiB。阅读时 K1/K2 翻页，短按 K3 打开操作菜单：
