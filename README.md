...
实机操作与按键事件的对应关系为：K3（下）对应 Linux 按键码 373
（`KEY_MODE`）；K2（确定）对应按键码 119（`KEY_PAUSE`）。此前同时按住
K2/K3 采集到约 552–553 mV 和 766 mV 两组值，不能仅凭采样顺序判断物理标签。
配置中的 `*_alt` 保留旧版键值作为兼容映射。

## 操作方式

- K1（上）：移动到上一项、切换应用或向前翻页
- K3（下，按键码 373）：移动到下一项、切换应用或向后翻页
- K2（确定，按键码 119）短按：打开应用、执行当前操作
...
开始手机配网时会先自动扫描附近网络，再创建热点。手机连接后，Android/iOS 的
联网检测会被引导到配网页；页面按信号强度列出 Wi-Fi，并默认选中信号最强的一项，
同时保留隐藏网络手动输入。若系统没有自动弹窗，浏览器访问上述地址即可。

选择网络或输入隐藏 SSID，再输入密码提交。
